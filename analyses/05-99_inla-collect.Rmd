...
```{r echo=FALSE}
final_male_year = final_male$m1$summary.random$id_year %>% 
  mutate(ID = ID + 2014, 
...
```{r echo=FALSE}
final_male_age = final_male$m1$summary.random$id_age %>% 
...
final_male_age %>% 
  ...
  theme_minimal() + 
  ylab("Age group") +
  scale_y_continuous(labels = unique(data_male$age)) +
  scale_x_log10()
```
...
```{r echo=FALSE}
ploterr(final_male_fit, "Schwende-Rüte")
```
...
```{r echo=FALSE}
final_female_year = final_female$m1$summary.random$id_year %>% 
  mutate(ID = ID + 2014, 
         `0.025quant` = exp(`0.025quant`),
         `0.5quant` = exp(`0.5quant`),
         `0.975quant` = exp(`0.975quant`))
...
```{r echo=FALSE}
final_female_age = final_female$m1$summary.random$id_age %>% 
  mutate(`0.025quant` = exp(`0.025quant`),
         `0.5quant` = exp(`0.5quant`),
         `0.975quant` = exp(`0.975quant`))
...
final_female_age %>% 
  ...
  theme_minimal() + 
  ylab("Age group") +
  scale_y_continuous(labels = unique(data_female$age)) +
  scale_x_log10()
```
