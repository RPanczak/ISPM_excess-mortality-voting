---
title: "2020 excess mortality & voting patterns in CH"
subtitle: "INLA models using GEM"
author: "Radek Panczak"
date: "`r Sys.Date()`"
output:
  rmdformats::robobook:
    code_folding: show
    self_contained: true
    highlight: pygments
editor_options: 
  chunk_output_type: console
knit: (function(inputFile, encoding) {
  rmarkdown::render(inputFile, encoding = encoding, output_dir = "../docs") })
---

<!-- ----------------------------------------------------- -->

```{r setup, include = FALSE}
set.seed(12345)
options(scipen = 999)
options(max.print = "75")

library(pacman)
p_load(tidyverse, magrittr, scales,  
       sf, tmap,
       INLA)

import::from("sjmisc", "frq")
```

```{r conflicts, include = FALSE}
tidyverse::tidyverse_conflicts()

conflicted::conflict_scout()
```

```{r knit-setup, include = FALSE}
knitr::opts_knit$set(root.dir = rprojroot::find_rstudio_root_file())

knitr::opts_chunk$set(cache = FALSE,
                      prompt = FALSE,
                      tidy = FALSE,
                      comment = NA,
                      message = FALSE,
                      warning = FALSE)

knitr::opts_knit$set(width = 75)
```

<!-- ----------------------------------------------------- -->

# Setup 

## Data

```{r}
data <- read_rds("data/BfS-closed/monthly_deaths/w_deaths_2015_2020_year_fin.Rds") %>% 
  filter(age != "<40") %>% 
  # strata with double zeroes seem to be crashing
  filter(pop_mid_poi > 0) 

data_male <- data %>% 
  filter(sex == "Male") %>% 
  select(-sex) %>% 
  as.data.frame()

data_female <- data %>% 
  filter(sex == "Female") %>% 
  select(-sex) %>% 
  as.data.frame()
```

## Spatial 

```{r}
kt <- readRDS("data/BfS/kt.Rds")
gg <- readRDS("data/BfS/gg.Rds")
tg3o <- readRDS("data/BfS/tg3o.Rds")
```

<!-- ----------------------------------------------------- -->

# Model comparison

```{r}
gem_sex_bym2 <- read_rds("results/gem_sex_bym2.Rds")
```

## DIC {.tabset}

### Males 

```{r}
dotchart(c(
  gem_sex_bym2$Male$m1$dic$dic,
  gem_sex_bym2$Male$m2$dic$dic,
  gem_sex_bym2$Male$m3$dic$dic,
  gem_sex_bym2$Male$m4$dic$dic
),
labels = c("Model1",
           "Model 2",
           "Model 3",
           "Model 4"))
```

### Females 

```{r}
dotchart(c(
  gem_sex_bym2$Female$m1$dic$dic,
  gem_sex_bym2$Female$m2$dic$dic,
  gem_sex_bym2$Female$m3$dic$dic,
  gem_sex_bym2$Female$m4$dic$dic
),
labels = c("Model1",
           "Model 2",
           "Model 3",
           "Model 4"))
```

## WAIC {.tabset}

### Males 

```{r}
dotchart(c(
  gem_sex_bym2$Male$m1$waic$waic,
  gem_sex_bym2$Male$m2$waic$waic,
  gem_sex_bym2$Male$m3$waic$waic,
  gem_sex_bym2$Male$m4$waic$waic
),
labels = c("Model1",
           "Model 2",
           "Model 3",
           "Model 4"))
```

### Females 

```{r}
dotchart(c(
  gem_sex_bym2$Female$m1$waic$waic,
  gem_sex_bym2$Female$m2$waic$waic,
  gem_sex_bym2$Female$m3$waic$waic,
  gem_sex_bym2$Female$m4$waic$waic
),
labels = c("Model1",
           "Model 2",
           "Model 3",
           "Model 4"))
```

<!-- ----------------------------------------------------- -->

# Best model

## Model 

```{r}
final_male <- gem_sex_bym2$Male
summary(final_male$m2)
final_male$m2$summary.hyperpar
```

## Year 

```{r}
final_male_year <- final_male$m2$summary.random$id_year %>% 
  mutate(ID = ID + 2014, 
         `0.025quant` = exp(`0.025quant`),
         `0.5quant` = exp(`0.5quant`),
         `0.975quant` = exp(`0.975quant`))

final_male_year %>% 
  ggplot(aes(y = ID)) + 
  geom_pointrange(aes(x = `0.5quant`, xmin = `0.025quant`, xmax = `0.975quant`)) + 
  theme_minimal() + 
  scale_x_log10() + 
  coord_flip() + 
  ylab("")
```

## Age 

```{r}
final_male_age <- final_male$m2$summary.random$id_age %>% 
  mutate(`0.025quant` = exp(`0.025quant`),
         `0.5quant` = exp(`0.5quant`),
         `0.975quant` = exp(`0.975quant`))

final_male_age %>% 
  ggplot(aes(y = ID)) + 
  geom_pointrange(aes(x = `0.5quant`, xmin = `0.025quant`, xmax = `0.975quant`)) + 
  theme_minimal() + 
  ylab("Age group") +
  scale_x_log10()
```

## Observed vs predicted

```{r}
stopifnot(nrow(final_male$m2$summary.fitted.values) == nrow(data_male))

final_male_fit <- bind_cols(
  data_male,
  final_male$m2$summary.fitted.values
) %>% 
  remove_rownames() %>% 
  mutate(split = ifelse(year < 2020, "Pre-pandemic", "Pandemic"))

ggplot(final_male_fit) + 
  geom_abline(intercept = 0, slope = 1, color = "darkorchid") +
  geom_point(aes(x = `0.5quant`, y = observed), alpha = 0.22) + 
  facet_wrap(vars(year)) +
  coord_fixed() +
  xlab("predicted") + 
  theme_minimal()
```

## Examples

### Lugano 

```{r}
final_male_fit %>% 
  filter(GMDNAME == "Lugano") %>% 
  ggplot(aes(x = year, fill = age, group = age)) + 
  geom_ribbon(aes(ymin = `0.025quant`, ymax = `0.975quant`), alpha = 0.1) +
  geom_line(aes(y = `0.5quant`, color = age)) + 
  geom_point(aes(y = observed, color = age), alpha = 0.5) + 
  # facet_wrap(vars(split), scales = "free_x") +
  xlab("") + ylab("predicted/observed") +
  theme_minimal()
```

### Most error 2015-19

```{r}
final_male_fit %>% 
  filter(year < 2020) %>% 
  mutate(diff = as.integer(round(observed - `0.5quant`))) %>% 
  group_by(GMDNAME) %>% 
  summarize(diff = abs(sum(diff))) %>% 
  ungroup() %>% 
  arrange(desc(diff))

final_male_fit %>% 
  filter(GMDNAME == "Schwende-Rüte") %>% 
  ggplot(aes(x = year, fill = age, group = age)) + 
  geom_ribbon(aes(ymin = `0.025quant`, ymax = `0.975quant`), alpha = 0.1) +
  geom_line(aes(y = `0.5quant`, color = age)) + 
  geom_point(aes(y = observed, color = age), alpha = 0.5) + 
  # facet_wrap(vars(split), scales = "free_x") +
  xlab("") + ylab("predicted/observed") +
  theme_minimal()

final_male_fit %>% 
  filter(GMDNAME == "Appenzell") %>% 
  ggplot(aes(x = year, fill = age, group = age)) + 
  geom_ribbon(aes(ymin = `0.025quant`, ymax = `0.975quant`), alpha = 0.1) +
  geom_line(aes(y = `0.5quant`, color = age)) + 
  geom_point(aes(y = observed, color = age), alpha = 0.5) + 
  # facet_wrap(vars(split), scales = "free_x") +
  xlab("") + ylab("predicted/observed") +
  theme_minimal()

final_male_fit %>% 
  filter(GMDNAME == "Goumoëns") %>% 
  ggplot(aes(x = year, fill = age, group = age)) + 
  geom_ribbon(aes(ymin = `0.025quant`, ymax = `0.975quant`), alpha = 0.1) +
  geom_line(aes(y = `0.5quant`, color = age)) + 
  geom_point(aes(y = observed, color = age), alpha = 0.5) + 
  # facet_wrap(vars(split), scales = "free_x") +
  xlab("") + ylab("predicted/observed") +
  theme_minimal()
```

### Least error 2015-19

```{r}
final_male_fit %>% 
  filter(year < 2020) %>% 
  mutate(diff = as.integer(round(observed - `0.5quant`))) %>% 
  group_by(GMDNAME) %>% 
  summarize(diff = abs(sum(diff)),
            deaths = sum(deaths)) %>% 
  ungroup() %>% 
  arrange(diff) 

final_male_fit %>% 
  filter(GMDNAME == "Adlikon") %>% 
  ggplot(aes(x = year, fill = age, group = age)) + 
  geom_ribbon(aes(ymin = `0.025quant`, ymax = `0.975quant`), alpha = 0.1) +
  geom_line(aes(y = `0.5quant`, color = age)) + 
  geom_point(aes(y = observed, color = age), alpha = 0.5) + 
  # facet_wrap(vars(split), scales = "free_x") +
  xlab("") + ylab("predicted/observed") +
  theme_minimal()

final_male_fit %>% 
  filter(GMDNAME == "Aeschi bei Spiez") %>% 
  ggplot(aes(x = year, fill = age, group = age)) + 
  geom_ribbon(aes(ymin = `0.025quant`, ymax = `0.975quant`), alpha = 0.1) +
  geom_line(aes(y = `0.5quant`, color = age)) + 
  geom_point(aes(y = observed, color = age), alpha = 0.5) + 
  # facet_wrap(vars(split), scales = "free_x") +
  xlab("") + ylab("predicted/observed") +
  theme_minimal()

final_male_fit %>% 
  filter(GMDNAME == "Baden") %>% 
  ggplot(aes(x = year, fill = age, group = age)) + 
  geom_ribbon(aes(ymin = `0.025quant`, ymax = `0.975quant`), alpha = 0.1) +
  geom_line(aes(y = `0.5quant`, color = age)) + 
  geom_point(aes(y = observed, color = age), alpha = 0.5) + 
  # facet_wrap(vars(split), scales = "free_x") +
  xlab("") + ylab("predicted/observed") +
  theme_minimal()
```

## Aggregating all observed and pred over time

```{r}
final_male_fit_agg <- final_male_fit %>% 
  group_by(year) %>% 
  summarise(observed = sum(observed),
            # population = sum(population),
            predicted = as.integer(sum(`0.5quant`))) %>% 
  ungroup() %>% 
  mutate(diff = observed - predicted)

final_male_fit_agg

final_male_fit_agg %>% 
  ggplot(aes(x = year)) + 
  geom_line(aes(y = predicted)) + 
  geom_line(aes(y = observed), col = "darkorchid", alpha = 0.5) + 
  # facet_wrap(vars(split), scales = "free_x") +
  xlab("") + ylab("predicted/observed") +
  theme_minimal()
```

## Region effect 

```{r}
length(data_male$id_space)
length(unique(data_male$id_space))

length(tg3o$GMDNR)
length(unique(tg3o$GMDNR))

length(final_male$m2$summary.random$id_space$ID)
length(unique(final_male$m2$summary.random$id_space$ID))

summary(final_male$m2$summary.random$id_space$ID)
```

```{r}
final_male_space <-
  tg3o %>%
  select(GMDNR) %>% 
  left_join(data_male %>% select(GMDNR, id_space) %>% distinct()) %>%
  left_join(final_male$m2$summary.random$id_space %>%
              mutate(s1_l = exp(`0.025quant`),
                     s1_m = exp(`0.5quant`),
                     s1_u = exp(`0.975quant`)) %>% 
              select(ID, starts_with("s1")),
            by = c("id_space" = "ID")) # %>%
```

```{r}
tm_shape(final_male_space) +
  tm_fill("s1_m", palette = "-BrBG", n = 6, midpoint = 1, 
          title = "Region effect") + 
  tm_shape(kt) +
  tm_borders(col = "grey60")
```

```{r}
final_male_space <-
  tg3o %>%
  select(GMDNR) %>% 
  left_join(data_male %>% 
              filter(year == 2020) %>% 
              group_by(GMDNR, id_space) %>% 
              summarise(pop_mid_poi = sum(pop_mid_poi)) %>% 
              ungroup()) %>%
  left_join(final_male$m2$summary.random$id_space %>%
              mutate(s1_l = exp(`0.025quant`),
                     s1_m = exp(`0.5quant`),
                     s1_u = exp(`0.975quant`)) %>% 
              select(ID, starts_with("s1")),
            by = c("id_space" = "ID")) # %>%
```

```{r}
tm_shape(kt) +
  tm_borders() +  
  tm_shape(final_male_space) +
  tm_symbols(size = "pop_mid_poi", col = "s1_m",
             shape = 15, scale = 2, perceptual = TRUE, 
             legend.size.show = FALSE, 
             palette = "-BrBG", n = 6, midpoint = 1, 
             title.size = "Population", 
             title.col = "Region effect") + 
  tm_legend(position = c("left", "top"))
```

## Performance 

```{r}
marg.variance <- inla.tmarginal(function(x) 1/x,
                                final_male$m2$marginals.hyperpar$"Precision for id_space")
inla.zmarginal(marg.variance)
```

```{r eval=FALSE, include=FALSE}
table(final_male$m2$cpo$failure > 0)
sum(-log(final_male$m2$cpo$cpo), na.rm = TRUE)
```


