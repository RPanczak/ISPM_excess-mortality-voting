---
title: "2020 excess mortality & voting patterns in CH"
subtitle: "Redistributed cantonal deaths"
author: "Radek Panczak"
date: "`r Sys.Date()`"
output:
  rmdformats::robobook:
    code_folding: show
    self_contained: true
    highlight: pygments
editor_options: 
  chunk_output_type: console
knit: (function(inputFile, encoding) {
  rmarkdown::render(inputFile, encoding = encoding, output_dir = "../docs") })
---

<!-- ----------------------------------------------------- -->

```{r setup, include = FALSE}
set.seed(12345)
options(scipen = 999)
options(max.print = "75")

library(pacman)
p_load(tidyverse, magrittr, skimr, scales,
       hrbrthemes, 
       sf, tmap)

import::from("sjmisc", "frq")
import::from("weights", "wtd.cor")
```

```{r conflicts, include = FALSE}
tidyverse::tidyverse_conflicts()

conflicted::conflict_scout()
```

```{r knit-setup, include = FALSE}
knitr::opts_knit$set(root.dir = rprojroot::find_rstudio_root_file())

knitr::opts_chunk$set(cache = FALSE,
                      prompt = FALSE,
                      tidy = FALSE,
                      comment = NA,
                      message = FALSE,
                      warning = FALSE)

knitr::opts_knit$set(width = 75)
```

<!-- ----------------------------------------------------- -->

# Data 

## Canton 

Expected deaths by age X sex X canton in 2020 from [Riou *et al.*](https://www.medrxiv.org/content/10.1101/2022.08.05.22278458v1).   

```{r include=FALSE}
exp_deaths_2020_kt = read_rds("data/canton/exp_deaths_2020_kt.rds") %>% select(-year)
```

Contains iterations of diff results  

```{r}
max(exp_deaths_2020_kt$it)
```

Example of one iteration  

```{r echo=FALSE}
exp_deaths_2020_kt %>% 
  filter(it == 1) %>% 
  skim()
```

## Municipality 

Data prepared in `02.Rmd`. Collapsing age groups and renaming variables to match cantonal estimates.  

```{r}
w_deaths_2020_year_fin = read_rds("data/BfS-closed/monthly_deaths/w_deaths_2015_2020_year_fin.Rds") %>% 
  filter(year == 2020) %>% select(-year) %>% 
  rename(age_group = age,
         canton = KTNAME) %>% 
  mutate(age_group = if_else(age_group == "40-49", "40-59", age_group),
         age_group = if_else(age_group == "50-59", "40-59", age_group)) %>% 
  group_by(canton, GMDNR, age_group, sex) %>% 
  summarise(observed = sum(observed),
            pop_mid_poi = sum(pop_mid_poi)) %>% 
  ungroup()
```

```{r echo=FALSE}
w_deaths_2020_year_fin %>% 
  skim()
```

## Spatial 

```{r}
kt = read_rds("data/BfS/kt.Rds")
gg = read_rds("data/BfS/gg.Rds")
tg3o = read_rds("data/BfS/tg3o.Rds")
se_alt = read_rds("data/BfS/se_alt.Rds")
```

## Downscale function 

@jriou method to downscale cantonal deaths to municipality level.  

```{r}
source("R/downscale_year.R")
```

<!-- ----------------------------------------------------- -->

# Downscale 

```{r}
exp_deaths_2020_year = downscale_year(exp_deaths_2020_kt, w_deaths_2020_year_fin)
```

Again, result contains iterations of diff results  

```{r}
max(exp_deaths_2020_year$it)
```

Example of one iteration  

```{r echo=FALSE}
exp_deaths_2020_year %>% 
  filter(it == 1) %>% 
  skim()
```

```{r include=FALSE}
rm(exp_deaths_2020_kt, w_deaths_2020_year_fin, downscale_year); gc()
```

<!-- ----------------------------------------------------- -->

# Preps

## Aggregates 

Iterations collapsed for each municipality strata.  

```{r}
exp_deaths_2020_year_agg = exp_deaths_2020_year %>% 
  rename(munici_observed = observed,
         cant_exp_deaths = exp_deaths,
         cant_observed = observed2) %>% 
  group_by(canton, GMDNR, age_group, sex) %>% 
  summarise(
    # observed
    munici_observed = first(munici_observed),
    pop_mid_poi = first(pop_mid_poi),
    # modelled x1000
    munici_expected_med = as.integer(quantile(munici_exp_deaths, 0.5)),
    munici_excess_lo = as.integer(quantile(munici_excess, 0.025)),
    munici_excess_med = as.integer(quantile(munici_excess, 0.5)),
    munici_excess_up = as.integer(quantile(munici_excess, 0.975))
  ) %>% 
  ungroup()
```

Even simpler aggregation to municipality level, ignoring age & sex strata.  

```{r}
exp_deaths_2020_year_gem = exp_deaths_2020_year_agg %>% 
  group_by(canton, GMDNR) %>% 
  summarise(
    # community level totals
    munici_excess_med = sum(munici_excess_med),
    munici_observed = sum(munici_observed),
    munici_expected_med = sum(munici_expected_med),
    pop_mid_poi = sum(pop_mid_poi)
  ) %>% 
  ungroup() %>% 
  # percent and per pop
  mutate(munici_excess_perc = (munici_excess_med / munici_observed) * 100,
         munici_excess_pop =  (munici_excess_med / pop_mid_poi) * 1000) %>% 
  # zero expected & nonzero observed
  mutate(munici_excess_perc = if_else(munici_expected_med == 0 & munici_observed != 0, 
                                      munici_observed * 100, 
                                      munici_excess_perc)) %>% 
  # nonzero expected & zero observed
  mutate(munici_excess_perc = if_else(munici_expected_med != 0 & munici_observed == 0, 
                                      munici_expected_med * 100 * -1, 
                                      munici_excess_perc)) %>% 
  # zero zero
  mutate(munici_excess_perc = if_else(munici_observed == 0 & munici_expected_med == 0, 
                                      0, 
                                      munici_excess_perc))
```


## Bring covariates 

```{r}
exp_deaths_2020_year_agg %<>% 
  left_join(read_rds("data/BfS-closed/monthly_deaths/w_deaths_2015_2020_year_fin.Rds") %>% 
              select(GMDNR, GMDNAME, border, 
                     median_ssep3_q, r_urban1, r_urban2, r_lang) %>% 
              distinct())

exp_deaths_2020_year_gem %<>% 
  left_join(read_rds("data/BfS-closed/monthly_deaths/w_deaths_2015_2020_year_fin.Rds") %>% 
              select(GMDNR, GMDNAME, border, 
                     median_ssep3_q, r_urban1, r_urban2, r_lang) %>% 
              distinct())
```

## Bring voting  

```{r}
covid_jun = read_rds("data/voting/covid_jun.Rds") %>% 
  select(GMDNR, jaStimmenInProzent, vote_yes) %>% 
  rename(vote_yes_jun_perc = jaStimmenInProzent,
         vote_yes_jun_cat = vote_yes)

covid_nov = read_rds("data/voting/covid_nov.Rds") %>% 
  select(GMDNR, jaStimmenInProzent, vote_yes) %>% 
  rename(vote_yes_nov_perc = jaStimmenInProzent,
         vote_yes_nov_cat = vote_yes)
```

Note: there are few small communities with no voting info - **these were excluded**.  

```{r echo=FALSE}
exp_deaths_2020_year_gem %>%
  anti_join(covid_jun) %>% 
  select(canton, GMDNAME)
```

```{r include=FALSE}
exp_deaths_2020_year_agg %<>% 
  left_join(covid_jun) %>% 
  filter(!is.na(vote_yes_jun_cat))

exp_deaths_2020_year_gem %<>% 
  left_join(covid_jun) %>% 
  filter(!is.na(vote_yes_jun_cat))

exp_deaths_2020_year_agg %<>% 
  left_join(covid_nov) %>% 
  filter(!is.na(vote_yes_nov_cat)) %>% 
  relocate(GMDNAME, .after = GMDNR)

exp_deaths_2020_year_gem %<>% 
  left_join(covid_nov) %>% 
  filter(!is.na(vote_yes_nov_cat)) %>% 
  relocate(GMDNAME, .after = GMDNR)

write_rds(exp_deaths_2020_year_agg, "results/exp_deaths_2020_year_agg.Rds")
write_rds(exp_deaths_2020_year_gem, "results/exp_deaths_2020_year_gem.Rds")
```

<!-- ----------------------------------------------------- -->

# EDA

## Excess deaths per 1000 pop

### Distribution 

```{r echo=FALSE}
ggplot(exp_deaths_2020_year_gem, aes(x = munici_excess_pop)) + 
  geom_histogram(binwidth = 0.5) + 
  xlab("Excess deaths / 1000") + ylab("Number of communities") + 
  theme_light()
```

### Maps

```{r include=FALSE}
temp =
  tg3o %>%
  select(GMDNR) %>% 
  right_join(exp_deaths_2020_year_gem) %>% 
  mutate(munici_excess_pop_cat = Hmisc::cut2(munici_excess_pop, g = 5))

tmap_mode("plot")
```

```{r echo=FALSE}
frq(temp$munici_excess_pop_cat)
```

```{r echo=FALSE}
tm_shape(se_alt, is.master = FALSE) +
  tm_fill(col = "#a7cdf2") + 
  tm_shape(temp, is.master = TRUE) +
  tm_fill("munici_excess_pop_cat", palette = "-PuOr", 
          title = "Escess / 1000") + 
  tm_shape(kt) +
  tm_borders(col = "grey60")
```

```{r echo=FALSE}
tm_shape(se_alt, is.master = FALSE) +
  tm_fill(col = "#a7cdf2") + 
  tm_shape(kt, is.master = TRUE) +
  tm_borders() +  
  tm_shape(temp) +
  tm_symbols(size = "pop_mid_poi", col = "munici_excess_pop_cat",
             shape = 15, scale = 2, perceptual = TRUE, 
             legend.size.show = FALSE, 
             palette = "-PuOr", 
             title.size = "Population", 
             title.col = "Escess / 1000") + 
  tm_legend(position = c("LEFT", "TOP"))
```

```{r echo=FALSE}
tmap_mode("view")

tm_shape(se_alt, is.master = FALSE) +
  tm_fill(col = "#a7cdf2") + 
  tm_shape(temp, is.master = TRUE) +
  tm_fill(c("munici_excess_pop_cat", "vote_yes_jun_cat"), 
            palette = list("-PuOr", "PuOr"), 
          title = "Excess & voting") + 
  tm_shape(kt) +
  tm_borders(col = "grey60")
```
  

### Correlations

#### Unweighted {.tabset}

```{r}
cor.test(exp_deaths_2020_year_gem$munici_excess_pop, 
         exp_deaths_2020_year_gem$vote_yes_jun_perc, 
         method = "pearson")
```

#### Weighted

```{r}
wtd.cor(exp_deaths_2020_year_gem$munici_excess_pop, 
        exp_deaths_2020_year_gem$vote_yes_jun_perc, 
        weight = exp_deaths_2020_year_gem$munici_observed)
```

## Scatter {.tabset}

### Unweighted

```{r echo=FALSE, warning=FALSE}
exp_deaths_2020_year_gem %>% 
  ggplot(aes(x = munici_excess_pop, y = vote_yes_jun_perc)) + 
  geom_smooth() +
  geom_point(alpha = 0.05) + 
  theme_ipsum_rc() +
  xlab("Excess deaths / 1,000") + ylab("% vote yes")
```

### Weighted

```{r echo=FALSE, warning=FALSE}
exp_deaths_2020_year_gem %>% 
  ggplot(aes(x = munici_excess_pop, y = vote_yes_jun_perc)) + 
  geom_smooth(aes(weight = munici_observed)) +
  geom_point(aes(size = munici_observed), alpha = 0.05) + 
  theme_ipsum_rc() +
  theme(legend.position = "none") +
  xlab("Excess deaths") + ylab("% vote yes")
```


