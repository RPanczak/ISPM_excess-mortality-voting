...
```{r}
ggplot(pop_compare, aes(x = pop_diff, y = perc_diff)) + 
...
tm_shape(gg_centr) +
  tm_bubbles(size = "pop_diff", col = "perc_diff",
             style = "fixed", breaks = c(-20, -10, -5, -1, 0, 1, 5, 10, 20))
```
...
```{r eval=FALSE, include=FALSE}
table(w_deaths_2015_2020_year_pop$year)
...
```{r echo=FALSE}
surplus %>% 
  select(-GMDNR, -border, -starts_with("AR")) %>% 
  datatable()
```
...
carto_data <- 
  ...
  left_join(
    pop_complete %>% 
      # using 2019 to avoid effects of pandemic 
...
Dataset consists of `r scales::number(nrow(statpop2020), big.mark = ",")` individuals and `r scales::number(nrow(statpop2020_agg), big.mark = ",")` buildings. We selected individuals with type of residence *Hauptwohnsitz*, pop type *Ständige Wohnbevölkerung*, main residency category *Nur ein Hauptwohnsitz* and with valid EGID building ID.  
...
```{r eval=FALSE, include=FALSE}
frq(ssep3_gem_neigh_geo$median_ssep3_d)
```
...
statpop2020_agg_sep_gem <- read_rds("data/BfS-closed/SEP/statpop2020_agg_sep_gem.Rds") %>% 
  st_drop_geometry()
```
...
```{r eval=FALSE, include=FALSE}
frq(ssep3_gem_pop_geo$median_ssep3_d)
...
The other way round - exist in the voting dataset but not on the map:  

```{r}
covid_jun %>% 
  anti_join(st_gg %>%  
              st_drop_geometry()) %>% 
  select(GMDNR, mun_id, mun_name)
```

FIXME with [#28](https://github.com/RPanczak/ISPM_excess-mortality-voting/issues/28)  
