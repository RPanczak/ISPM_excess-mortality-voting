...
```{r eval=FALSE, include=FALSE}
st_write(gg, "data/nb/nb.gpkg", layer = "gg", append = FALSE)
st_write(tg3o, "data/nb/nb.gpkg", layer = "tg3o", append = FALSE)
```
...
```{r}
gg_nb <- st_read("data/nb/nb.gpkg", layer = "gg_nb")
```

## Weights

```{r}
gg_wm_q <- poly2nb(gg_nb, queen = TRUE, row.names = gg$GMDNR)
...
```{r}
tg3o_nb <- st_read("data/nb/nb.gpkg", layer = "tg3o_nb")
```
